    ...
    for a, b in permutations(zip(field_data, single_field_features), 2):
        ...
        try:
            ...
            while time() < (start_time + timeout):
                name_pairwise_features = get_name_pairwise_features(a_data['name'], b_data['name'])
                statistical_pairwise_features = get_statistical_pairwise_features(a_data, b_data)
                general_pairwise_features = get_general_pairwise_features(a_data, b_data)
                break
        except Exception as e:
            print('Error getting features for {} and {}'.format(a_data['name'], b_data['name']))
            continue
        
        pairwise_field_features = { }
        pairwise_field_features.update(general_pairwise_features)
        pairwise_field_features.update(statistical_pairwise_features)
        pairwise_field_features.update(name_pairwise_features)

        pairwise_field_features['fid'] = fid
        pairwise_field_features['field_a_id'] = a_features['field_id']
        pairwise_field_features['field_b_id'] = b_features['field_id']
        pairwise_field_features['pair_exists'] = True

        all_pairwise_field_features.append(pairwise_field_features)
